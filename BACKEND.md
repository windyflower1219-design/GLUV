...
*   **AI 및 자연어 처리**: OpenAI (ChatGPT-4o-mini) & Google Gemma 4 API (하이브리드 아키텍처)
...
*   `api/insights/route.ts` (Gemma 4): 입력된 최근 식단과 평균 혈당 수치를 분석하여 사용자에게 공감형/추천형/경고형 인사이트를 동적 생성해주는 서버 API 엔진.
...
*   **환경 변수 관리**: Vercel 프로젝트 Settings > Environment Variables에서 다음 3가지 핵심 설정 그룹이 필요합니다.
    ...
    *   `NEXT_PUBLIC_GEMINI_API_KEY`: 주간 데이터 수집 및 공감/조언 인사이트용(Gemma 4 호출용) 모델 키
